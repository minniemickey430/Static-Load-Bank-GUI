...
- Aydınlık tema, emoji yok
- Same-COM (tek port, çok adres) — güvenli port kilidi
- Port "takılı kalma" önlemleri: global kilit + temiz kapatma + retry
- NTC: N0..N19, threshold ayarlanabilir (vars. 40°C), min/max GUI içi
- CSV: periyodik (kullanıcı saniye)
- Röle: ALL ON (static) 15 ms aralıklı
"""
...
# ------------------ Port Registry (global kilit) ------------------
class PortRegistry:
    """Aynı COM'u aynı anda iki yerde açmayı engeller."""
    def __init__(self):
        self.locked: Dict[str, str] = {}  # port -> owner string

    def acquire(self, port: str, owner: str) -> bool:
        port = (port or "").upper()
        if not port: return False
        if port in self.locked and self.locked[port] != owner:
            return False
        self.locked[port] = owner
        return True

    def release(self, port: str, owner: str):
        port = (port or "").upper()
        if not port: return
        if self.locked.get(port) == owner:
            del self.locked[port]
...
class SerialClient:
    ...
    def open(self):
        # Önce varsa thread/port temizle
        if self._th and self._th.is_alive():
            ...
            time.sleep(0.1)  # Windows handle release

        if not self.port:
            raise RuntimeError("Select a port first")

        # PermissionError için kısa retry
        last_err=None
        for attempt in range(1,4):
            try:
                ...
                last_err=e
                time.sleep(0.2*attempt)  # kademeli bekleme
            except Exception as e:
                last_err=e
                break
        if not self.ser or not self.ser.is_open:
            raise last_err or RuntimeError("Port open failed")
    ...
    def close(self):
        ...
        try:
            ...
            if th and th.is_alive(): th.join(timeout=0.8)
            self.ser = None
        self.log("[CLOSE]")
        time.sleep(0.1)  # Windows handle release
...
class SerialHub:
    ...
    def refresh_addr_map(self):
        ...
        for p in self.app.panels:
            try: mapping[int(p.addr_var.get()) & 0xFF] = p
            except: pass
    ...
    def _log(self, s:str):
        if self.app.panels: self.app.panels[0].log(s)

    def _hex(self, s:str):
        if self.app.panels: self.app.panels[0].hex_log(s)
    ...
    def _rx_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        if target:
            ...
            try: target.on_frame_bin(addr, cmd, sub, a1, a2, data)
            except Exception as e: target.log(f"[HUB RX ERR] {e}")
        else:
            if self.app.panels:
                self.app.panels[0].log(f"[HUB] Unmapped ADDR=0x{addr:02X} cmd=0x{cmd:02X}")

    def _rx_line(self, line:str):
        for p in self.app.panels: p.on_line_text(line)
...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        self.csv_interval_var = tk.IntVar(value=1)
        self._csv_job = None
    ...
    def _init_style(self):
        ...
        bg = "#f4f5f7"; card_bg = "#ffffff"; card_bd = "#d1d5db"; txt = "#111827"
        acc = "#2563eb"; acc_act = "#1d4ed8"; danger = "#dc2626"; danger_act = "#b91c1c"
    ...
    def set_shared_mode(self, on: bool):
        if on:
            ...
            try:
                if self.client and self.client.is_open(): self.client.close()
            except: pass
            self.client = None
        else:
    ...
    def _build_ui(self):
        ...
        rightwrap = ttk.Frame(bar); rightwrap.grid(row=0, column=1, padx=10, sticky="e")
        ...
        mm=ttk.Frame(ntcf); mm.grid(row=4, column=0, columnspan=5, sticky="ew", pady=(2,6))
        self.min_var=tk.StringVar(value="Min: -"); self.max_var=tk.StringVar(value="Max: -")
        ttk.Label(mm,textvariable=self.min_var).pack(side='left',padx=8)
        ttk.Label(mm,textvariable=self.max_var).pack(side='left',padx=8)
        ttk.Button(mm,text="Read Temps",command=self.read_temps,style="Tool.TButton").pack(side='right', padx=8)
        ...
        rfrm.grid_columnconfigure(0, weight=1); rfrm.grid_columnconfigure(1, weight=1)
        ...
        opt=ttk.Frame(rfrm); opt.grid(row=1, column=0, columnspan=2, sticky="ew", padx=8, pady=(0,10))
        ...
        ttk.Label(opt, text="Note: In electronic–static pairs, only one electronic channel may be ON at a time.").pack(side='right', padx=8)
        ...
        ttk.Button(btnrow,text="Sleep",command=self.oled_sleep,style="Tool.TButton").pack(side='left', padx=(0,8))
        ttk.Button(btnrow,text="Wake", command=self.oled_wake, style="Tool.TButton").pack(side='left')
    ...
    def _on_threshold_change(self):
        try: thr = float(self.ntc_threshold_var.get())
        except Exception:
            thr = 40.0; self.ntc_threshold_var.set(thr)
        self.NTC_LIMIT = int(round(thr * 10.0))
        self._ntc_dirty = set(range(20)); self._schedule_ntc_flush()

    def refresh_ports(self):
        ports=[p.device for p in list_ports.comports()]
        if ports:
            self.port_cb['values']=ports
            if not self.app.same_port_var.get(): self.btn_connect.configure(state='normal')
            self.log(f"[PORT] {len(ports)} port: {', '.join(ports)}")
        else:
            self.port_cb['values'] = ["(no ports)"]; self.port_cb.set("(no ports)")
            if not self.app.same_port_var.get(): self.btn_connect.configure(state='disabled')
            self.log("[PORT] 0 port bulundu")

    def connect(self):
        if self.app.same_port_var.get(): return  # shared modda panel bağlanmaz
        port=self.port_var.get().strip()
        if not port or port=="(no ports)":
            messagebox.showerror('Port','Select a port first'); return

        # Port kilidi
        owner=f"panel-{self.idx+1}"
        if not self.app.port_registry.acquire(port, owner):
            messagebox.showerror("Port busy", f"{port} is already in use by another drawer/shared.")
            return
        ...
        def worker():
            try:
                ...
                self.client.open()
                self.after(0, self.set_fans_auto_startup); self.after(0, self.oled_wake)
                self._log_thread("[INFO] Connected."); self.set_status("OK")
            except Exception as e:
                self.after(0, lambda: (self.btn_connect.configure(state='normal'),
                                        self.btn_disconnect.configure(state='disabled'),
                                        messagebox.showerror('Connect', f'{e}')))
                self.set_status(f"Error: {e}")
                # kilidi bırak (açamadı)
                self.app.port_registry.release(port, owner)
        threading.Thread(target=worker,daemon=True).start()

    def disconnect(self):
        if self.app.same_port_var.get(): return
        try:
            if self.client: self.client.close()
        except: pass
        # kilidi bırak
        self.app.port_registry.release(self.port_var.get().strip(), f"panel-{self.idx+1}")
        self.client = None
        self.btn_disconnect.configure(state='disabled'); self.btn_connect.configure(state='normal')
    ...
    def send_and_log(self, frame:bytes, note:str):
        if self.app.same_port_var.get():
            ...
            if not (self.client and self.client.is_open()):
                self.hex_log(f"TX[{port}] {hex_dump(frame)}  ;; NOT SENT"); return
            self.hex_log(f"TX[{port}] {hex_dump(frame)}  ;; {note}")
            self.client.send(frame)

    def set_fans_auto_startup(self):
        for v in self.fan_vars: v.set("AUTO")
    ...
    def _oled_can_fire(self)->bool:
        ...
        if now_ms - self._oled_last_ms < OLED_DEBOUNCE_MS:
            self.set_status("Warning: OLED command ignored (debounce)"); return False
        self._oled_last_ms = now_ms; return True

    def oled_sleep(self):
        if not self._oled_can_fire(): return
        self.send_and_log(pack(self._addr(), CMD_OLED, bytes([SUB_OLED_SLEEP])), "OLED -> SLEEP")

    def oled_wake(self):
        if not self._oled_can_fire(): return
        self.send_and_log(pack(self._addr(), CMD_OLED, bytes([SUB_OLED_WAKE])), "OLED -> WAKE")

    def on_click_static_channel(self, ch:int):
        rid = 2 * ch; idx = rid - 1
        want_on = (self.relays[idx] == 0)
        self.relays[idx] = 1 if want_on else 0; self._paint(idx)
        self.send_and_log(pack(self._addr(), CMD_RLY, bytes([SUB_RLY_ON if want_on else SUB_RLY_OFF, rid])),
                          f"Static ch={ch} (rid={rid}) -> {'ON' if want_on else 'OFF'}")

    def on_click_electronic_channel(self, ch:int):
        rid = 2 * ch - 1; idx = rid - 1
        want_on = (self.relays[idx] == 0)
        if want_on:
            for r in range(1,41,2):
                j = r - 1
                if j != idx and self.relays[j] != 0:
                    self.relays[j] = 0; self._paint(j)
        self.relays[idx] = 1 if want_on else 0; self._paint(idx)
        self.send_and_log(pack(self._addr(), CMD_RLY, bytes([SUB_RLY_ON if want_on else SUB_RLY_OFF, rid])),
                          f"Electronic ch={ch} (rid={rid}) -> {'ON' if want_on else 'OFF'}")

    def all_off(self):
        for k in range(40):
            if self.relays[k]!=0: self.relays[k]=0; self._paint(k)
        self.send_and_log(pack(self._addr(), CMD_RLY, bytes([SUB_RLY_OFF, 0x00])), "ALL OFF")

    def _schedule_frames_spaced(self, frames_notes: List[tuple], gap_ms:int=15):
        def send_next(i:int):
            if i>=len(frames_notes): return
    ...
    def on_fan_select(self, _idx:int):
        ...
        self.send_and_log(pack(self._addr(), CMD_FAN, bytes([sub])), f"FAN -> {label}")
    ...
    def _flush_ntc_ui(self):
        self._ntc_flush_job = None
        if not self._ntc_dirty: return
        dirty = list(self._ntc_dirty); self._ntc_dirty.clear()
        for i in dirty: self._set_ntc_tile(i, self.ntc_vals[i], only_if_changed=True)
        self._recompute_minmax()

    def _recompute_minmax(self):
        indices = [i for i, s in enumerate(self.ntc_seen) if s]
        if not indices:
            self.min_var.set("Min: -"); self.max_var.set("Max: -"); return
        min_idx = min(indices, key=lambda i: self.ntc_vals[i])
        max_idx = max(indices, key=lambda i: self.ntc_vals[i])
        self.ntc_min = self.ntc_vals[min_idx]; self.ntc_max = self.ntc_vals[max_idx]
        self.min_var.set(f"Min: {self.ntc_min/10.0:.1f} °C (N{min_idx})")
        self.max_var.set(f"Max: {self.ntc_max/10.0:.1f} °C (N{max_idx})")

    def start_csv(self):
        if self.csv_enabled:
            messagebox.showinfo('CSV','Already logging'); return
        path=filedialog.asksaveasfilename(defaultextension='.csv', filetypes=[('CSV','*.csv')],
                                          initialfile=f"drawer{self.idx+1}_log.csv")
        if not path: return
        try:
            self.csv_file=open(path,'w',newline='',encoding='utf-8')
        except Exception as e:
            messagebox.showerror("CSV", f"File open error: {e}"); return
        self.csv_writer=csv.writer(self.csv_file)
        header=['ts']+[f"R{i+1}" for i in range(40)]+[f"N{i}" for i in range(20)]+['NTC_MIN','NTC_MAX']+['FAN1','FAN2','FAN3','FAN_MODE','OLED']
        self.csv_writer.writerow(header); self.csv_enabled=True
    ...
    def _csv_snapshot_write(self):
        if not (self.csv_enabled and self.csv_writer): return
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        if cmd==CMD_ADDR and len(data)>=2:
            val=data[1]; self.addr_var.set(val); self.log(f"[ADDR] device reports address=0x{val:02X}"); return

        if cmd==CMD_EVT:
            if sub == 0x00 and a1 == 0xE8:
                self.log("[EVT] System Reset")
                for i in range(40):
                    if self.relays[i]!=0: self.relays[i]=0; self._paint(i)
                self.set_status("Warning: system reset"); return
            msg = SYS_STATUS_MAP.get(a1)
            if msg: self.log_error(f"[EVT] 0x{a1:02X} → {msg}")
            else:
                for i in range(40):
                    if self.relays[i]!=0: self.relays[i]=0; self._paint(i)
                self.set_status("Warning: system event (init/reset?)"); self.log("[EVT] System event received (init/reset?)")
            return

        if addr != (self._addr() & 0xFF): return

        if cmd==CMD_RLY:
            if sub==SUB_RLY_ERR:
                self.log_error(f"[RLY][ERR] Possible Holt/relay fault (arg1=0x{a1:02X})"); return
            ...
            if st==FAN_STATUS_MANUAL_OK:
                ...
                self.set_status(f"Warning: Fan status 0x{st:02X}"); self.log(f"[FAN] status=Unknown (0x{st:02X})")

        elif cmd==CMD_TMP:
            if sub in (SUB_TMP_MIN, SUB_TMP_MAX): return
            idx = sub; val_decic = a1
            if 0 <= idx < 20: self._update_ntc_value(idx, val_decic)
    ...
    def on_line_text(self,line:str):
        ...
        if m:
            ...
            if 0<=i<20:
                self.ntc_seen[i]=True; self.ntc_vals[i]=int(round(val*10))
                self._ntc_dirty.add(i); self._schedule_ntc_flush()

    def _set_ntc_tile(self,i:int, val:int, only_if_changed:bool=False):
        f,l1,l2=self.ntc_tiles[i]
        deg_text = f"{val/10.0:.1f} °C"
        if (not only_if_changed) or (l2.cget("text") != deg_text): l2.configure(text=deg_text)
        hot=(val>self.NTC_LIMIT); want_bg = "#fde2e2" if hot else "#dcfce7"
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.geometry('1320x940'); self.minsize(1100, 800); self['background']="#f4f5f7"

        self.port_registry = PortRegistry()  # <<< global port kilidi
        ...
        for i in range(5):
            ...
            p=DevicePanel(container,i,self); p.pack(fill='both', expand=True)
            self.nb.add(container,text=f"Drawer {i+1}"); self.panels.append(p)
    ...
    def on_same_toggle(self):
        ...
        if on:
            # bireysel bağlantıları kapat + kilitleri bırak
            for p in self.panels:
                try:
                    if p.client and p.client.is_open(): p.client.close()
                except: pass
                self.port_registry.release(p.port_var.get().strip(), f"panel-{p.idx+1}")
                p.client = None; p.set_shared_mode(True)
            shared_port = self.panels[0].port_var.get()
            if shared_port:
                for p in self.panels: p.port_var.set(shared_port)
            self.btn_shared_connect.configure(state='normal'); self.btn_shared_disconnect.configure(state='normal')
        else:
            self.shared_disconnect()
            for p in self.panels: p.set_shared_mode(False)
            self.btn_shared_connect.configure(state='disabled'); self.btn_shared_disconnect.configure(state='disabled')

    def on_panel_port_changed(self, panel:DevicePanel):
        if self.same_port_var.get():
            val = panel.port_var.get()
            for p in self.panels: p.port_var.set(val)

    def shared_connect(self):
        if not self.same_port_var.get():
            messagebox.showinfo("Info", "Enable 'Same COM for all drawers' first."); return

        # tüm bireysel bağlantılar kapalı ve kilitleri bırakılmış olmalı
        for p in self.panels:
            try:
                if p.client and p.client.is_open(): p.client.close()
            except: pass
        ...
        # shared kilidi al
        if not self.port_registry.acquire(port, "shared"):
            messagebox.showerror("Port busy", f"{port} is already in use by a drawer/shared."); return
        ...
        try:
            ...
            self.port_registry.release(port, "shared")
            messagebox.showerror('Connect (Shared)', f'{e}')

    def shared_disconnect(self):
        try: self.hub.close()
        except: pass
        # shared kilidini bırak
        self.port_registry.release(self.panels[0].port_var.get().strip(), "shared")
        for p in self.panels:
            p.set_status("Disconnected (shared)"); p.log("[INFO] Shared Disconnected.")
    ...
    def scan_ports(self):
        ...
        for p in self.panels:
            if ports:
                ...
                if not self.same_port_var.get(): p.btn_connect.configure(state='normal')
            else:
                p.port_cb['values'] = ["(no ports)"]; p.port_cb.set("(no ports)")
                if not self.same_port_var.get(): p.btn_connect.configure(state='disabled')
        if ports:
