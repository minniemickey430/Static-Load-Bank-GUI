...
# Sıcaklık
SUB_TMP_ALL = 0x00
SUB_TMP_MAX = 0x01  # (kullanma; min/max GUI'de)
SUB_TMP_MIN = 0x02  # (kullanma; min/max GUI'de)
SUB_TMP_ONE = 0x03
...
class SerialHub:
    ...
    def open(self, port:str):
        ...
        self.close()
        self.client = SerialClient(port, BAUD, 0, self._rx_bin, self._rx_line, self._log, self._hex)
        self.client.open()
    ...
    def _rx_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        if target:
            ...
            if self.app.panels: self.app.panels[0].log(f"[HUB] Unmapped ADDR=0x{addr:02X} cmd=0x{cmd:02X}")
...
class DevicePanel(ttk.Frame):
    ...
    def _on_threshold_change(self):
        ...
        self._repaint_all_ntc_tiles()  # sadece renk/metin günceller; UI yeniden oluşturmaz
    ...
    def connect(self):
        if self.app.same_port_var.get():
            if not self.app.closing: messagebox.showinfo("Shared", "Same COM mode: use top-bar Connect (Shared).")
            return
        port=self.port_var.get().strip()
        if not port or port=="(no ports)":
            if not self.app.closing: messagebox.showerror('Port','Select a port first')
            return
        try:
        ...
        def worker():
            try:
                self.client=SerialClient(port, BAUD, self._addr(),
    ...
    def disconnect(self):
        if self.app.same_port_var.get():
            if not self.app.closing: messagebox.showinfo("Shared", "Same COM mode: use top-bar Disconnect (Shared).")
            return
        try:
            if self.client: self.client.close()
        except: pass
    ...
    # ---- Sıcaklık ----
    def read_temps(self):
        # Sadece ALL tetiklersin; UI öğeleri yeniden oluşturulmaz
    ...
    def _recompute_minmax(self):
        valid = [i for i, s in enumerate(self.ntc_seen) if s]
        if not valid:
            self.min_var.set("Min: -"); self.max_var.set("Max: -"); return
        min_idx = min(valid, key=lambda i: self.ntc_vals[i])
        max_idx = max(valid, key=lambda i: self.ntc_vals[i])
    ...
    # ---- CSV (yalnızca snapshot; otomatik sorgu yok) ----
    def start_csv(self):
        if self.csv_enabled:
        ...
        # periyodik snapshot
        self._schedule_next_csv()

    def _schedule_next_csv(self):
        ...
        self._csv_job = self.after(interval_ms, self._csv_tick)
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        if cmd==CMD_ADDR and len(data)>=2:
            val=data[1]; self.addr_var.set(val)
            self.log(f"[ADDR] device reports address=0x{val:02X}"); return

        if cmd==CMD_EVT:
            # IWDG reset: sub=0x00, a1=0xE8
    ...
    def _set_ntc_tile(self,i:int, val:int):
        ...
        f.configure(bg=("#fde2e2" if hot else "#dcfce7"))  # soft red / soft green
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.geometry('1320x940'); self.minsize(1100, 800)
    ...
    def on_same_toggle(self):
        ...
        if on:
            ...
            for p in self.panels:
                if shared_port: p.port_var.set(shared_port)
