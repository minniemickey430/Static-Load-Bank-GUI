...
class SerialClient:
    ...
    def close(self):
        # Thread'e dur talimatı
        self._stop.set()
        try:
            if self.ser:
                # Buffer’ları boşalt (bazı sürücülerde şart)
                try:
                    self.ser.reset_input_buffer()
                    self.ser.reset_output_buffer()
                except:
                    pass
                # Okuma/yazmayı iptal et (destekliyorsa)
                try:
                    if hasattr(self.ser, "cancel_read"):  self.ser.cancel_read()
                    if hasattr(self.ser, "cancel_write"): self.ser.cancel_write()
                except:
            ...
            # Thread’i bekle ve referansları bırak
            th = self._th
            self._th = None
...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        self.relays=[0]*40
        self.ntc_vals=[0]*20           # 1 LSB = 0.1°C (decicelsius*10)
        self.ntc_seen=[False]*20       # hangi indeksler güncel değer aldı
        self.ntc_min=0; self.ntc_max=0

        self.fans=[0,0,0]

        # NTC threshold (kullanıcı değiştirilebilir) — varsayılan 40.0°C
        ...
        self._csv_job = None        # after() job id
        self.csv_event_mode = False # event-tetikli mod kapalı (periyodik kullan)
    ...
    def start_csv(self):
        ...
        # periyodik yazmayı başlat
        self._schedule_csv_next()

    def _schedule_csv_next(self):
        # Önce mevcut job varsa iptal et
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        if cmd==CMD_RLY:
            ...
            # GUI içi min/max — sadece ALL akışındaki tek tek değerleri kullanıyoruz
            if sub in (SUB_TMP_MIN, SUB_TMP_MAX):
                return
            # ALL akışı: sub=idx (0..19), a1=temp_decic
            idx = sub
            val_decic = a1
            if 0 <= idx < 20:
...
class App(tk.Tk):
    ...
    def _on_close(self):
        ...
        for p in getattr(self, "panels", []):
            try:
                p.stop_csv()         # CSV dosyasını kapat
                p.disconnect()
            except:
                pass
        try:
