# -*- coding: utf-8 -*-
"""
Static Load Bank GUI — Two Writers (GUI + DLL) with Global Named Mutex
- Global named mutex: "Global\\S2350_TX"  (Windows). C# DLL tarafıyla aynı olmalı.
- Monitor mode YOK. Hem GUI hem DLL komut gönderebilir; yazımlar mutex ile sıraya girer.
"""
...
try:
    ...
    from serial import serial_for_url  # <-- TCP/IP URL desteği
...
class GlobalMutex:
    """
    Windows'ta gerçek named mutex ("Global\\S2350_TX").
    Diğer OS'lerde no-op (True döner).
    """
    NAME = r"Global\S2350_TX"
    ...
    def __init__(self):
        ...
        if self._is_win:
            ...
            h = self.k32.CreateMutexW(None, False, self.NAME)
            if not h:
    ...
    def acquire(self, timeout_ms: int = 300) -> bool:
        ...
        res = self.k32.WaitForSingleObject(self._hdl, timeout_ms)
        return res in (WAIT_OBJECT_0, WAIT_ABANDONED)

    def release(self):
        if not self._is_win: return
        try: self.k32.ReleaseMutex(self._hdl)
        except: pass

    def close(self):
        if not self._is_win: return
        try:
            if self._hdl: self.k32.CloseHandle(self._hdl)
        except: pass
        self._hdl = None

GLOBAL_TX_MUTEX = GlobalMutex()
...
class SerialClient:
    ...
    def open(self):
        ...
        if lower.startswith(("socket://", "rfc2217://", "telnet://", "spy://")):
            self.ser = serial_for_url(self.port, self.baud, timeout=0.05, write_timeout=0.2)
        else:
    ...
    def send(self,blob:bytes):
        """
        İki master (GUI + DLL) aynı hattan yazabilir; burada Global Mutex ile sıraya gireriz.
        """
        if not self.is_open():
            self.hexlog(f"TX[{self.port or 'DISCONNECTED'}] {hex_dump(blob)}  ;; NOT SENT")
            return
        # ---- critical section ----
        if not GLOBAL_TX_MUTEX.acquire(timeout_ms=300):
            self.log("[MUTEX] acquire timeout → skip TX")
            return
        try:
            self.ser.write(blob)
...
class SerialHub:
    ...
    def send_from_panel(self, panel:'DevicePanel', frame:bytes, note:str):
        ...
        if self.client:
            self.client.send(frame)   # SerialClient.send zaten mutex kullanıyor
...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        # --- TCP/IP desteği ---
        self.use_tcpip = False
        self.ip_var = tk.StringVar(value="192.168.1.100")
        self.tcp_port_var = tk.IntVar(value=4001)  # NPort port-1 default
        ...
        # CSV periyodik loglama (değer CSV'si)
    ...
    def _build_ui(self):
        ...
        ttk.Label(head,text="CSV Log Interval (s):",style='Header.TLabel').grid(row=0, column=8, sticky="e")
        ...
        ttk.Label(head, text="IP:", style='Header.TLabel').grid(row=0, column=13, sticky="e")
        self.ent_ip = ttk.Entry(head, textvariable=self.ip_var, width=14, state='disabled')
        self.ent_ip.grid(row=0, column=14, padx=(6,8))

        ttk.Label(head, text="TCP Port:", style='Header.TLabel').grid(row=0, column=15, sticky="e")
        self.ent_tcp = ttk.Entry(head, textvariable=self.tcp_port_var, width=7, state='disabled')
        self.ent_tcp.grid(row=0, column=16, padx=(6,8))

        self.btn_tcp_toggle = ttk.Button(head, text="TCP/IP Modu", command=self._toggle_tcpip, style="Tool.TButton")
        self.btn_tcp_toggle.grid(row=0, column=17, padx=(6,8))
        ...
        fanf=ttk.LabelFrame(rightcol,text=" Synced Fans",style='Card.TLabelframe')
    ...
    def _compose_tcp_url(self) -> str:
        ...
        try:
            ...
            port = 4001
        if not ip:
            raise ValueError("Geçerli bir IP giriniz.")
        if port <= 0 or port > 65535:
            raise ValueError("Geçerli bir TCP port giriniz.")
        return f"socket://{ip}:{port}"  # RFC2217 için: f"rfc2217://{ip}:{port}"

    def _apply_tcpip_ui(self):
        if self.use_tcpip:
            self.port_cb.configure(state='disabled')
            self.ent_ip.configure(state='normal')
            self.ent_tcp.configure(state='normal')
        else:
            self.port_cb.configure(state='readonly')
            self.ent_ip.configure(state='disabled')
            self.ent_tcp.configure(state='disabled')

    def _toggle_tcpip(self):
        self.use_tcpip = not self.use_tcpip
        self._apply_tcpip_ui()
    ...
    def refresh_ports(self):
        if self.use_tcpip:
            self.log("[PORT] TCP/IP mode: COM scan skipped")
            return
    ...
    def connect(self):
        ...
        if self.use_tcpip:
            try:
                url = self._compose_tcp_url()
            except Exception as e:
                if not self.app.closing:
                    messagebox.showerror('TCP/IP', f'{e}')
                return
    ...
    def _csv_poll(self):
        ...
        # istersen röle durumu sorgusunu da açabilirsin:
...
class App(tk.Tk):
    ...
    def scan_ports(self):
        ports=[p.device for p in list_ports.comports()]
        for p in self.panels:
            if getattr(p, 'use_tcpip', False):
                p.log("[PORT] TCP/IP mode: global COM scan skipped for this panel")
                continue
            if ports:
