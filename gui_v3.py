...
- Tamamen metin tabanlı başlıklar (emoji yok)
- Aydınlık tema
...
- NTC UI: flicker önleme (coalesced flush), sabit içerik boyutu
"""
...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        self.relays=[0]*40
        self.ntc_vals=[0]*20           # 1 LSB = 0.1°C *10 => decicelsius
    ...
    def _build_ui(self):
        ...
        # NTC eşik (kullanıcı değiştirilebilir)
    ...
    def _on_threshold_change(self):
        ...
        # tam yeniden boyamak yerine sadece renk değişmesi gerekenleri güncelle
    ...
    def _flush_ntc_ui(self):
        ...
        # Renk ve metinler sadece değiştiyse güncellensin
        for i in dirty:
            self._set_ntc_tile(i, self.ntc_vals[i], only_if_changed=True)

        # Min/Max tek seferde hesapla
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        if cmd==CMD_RLY:
            ...
            # Sadece ALL akışındaki tek tek değerler
    ...
    def _set_ntc_tile(self,i:int, val:int, only_if_changed:bool=False):
        """Kutunun sadece gerekli parçasını güncelle (flicker azaltma)."""
        ...
        want_bg = "#fde2e2" if hot else "#dcfce7"  # soft red / soft green
        if (not only_if_changed) or (f.cget("bg") != want_bg):
            f.configure(bg=want_bg)
            # alt label arka planlarını da senkronize et
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.closing = False  # kapanışta uyarı pencerelerini bastırmak için
