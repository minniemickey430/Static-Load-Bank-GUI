...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        # --- TCP/IP desteği (COM mantığını bozmayacak) ---
    ...
    def _build_ui(self):
        ...
        # --- TCP/IP alanları ve toggle (CSV kontrollerinden sonra ekledik) ---
    ...
    def refresh_ports(self):
        # TCP/IP modunda COM taramayı atla (UI donmasın)
        if self.use_tcpip:
    ...
    def disconnect(self):
        ...
        self.client = None
        self.btn_disconnect.configure(state='disabled')
        self.btn_connect.configure(state='normal')   # <<< parantez düzeltildi
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        if cmd==CMD_RLY:
            ...
            # Threshold set yanıtı: AA ADDR 20 02 06 00 CRC 55  → a1==0x00 ⇒ OK
            if sub == 0x06:
                if a1 == 0x00:
            ...
            # FW per-index cevaplarda sub = 0..19 (N0..N19)
            idx = sub
            val_decic = a1
            if 0 <= idx < 20:
