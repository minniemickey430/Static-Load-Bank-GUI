...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int):
        ...
        # NTC eşiği sabit: 60.0°C
        self.NTC_LIMIT = 600  # 60.0°C (sabit, değiştirilemez)
    ...
    def _build_ui(self):
        ...
        # NTC eşik bilgisi (sabit 60°C)
        thrf=ttk.Frame(self, style="Card.TLabelframe")
        thrf.grid(row=2, column=0, sticky="ew", padx=8, pady=(0,6))
        ttk.Label(thrf, text="NTC Threshold: 60.0 °C (sabit)").pack(side='left', padx=(12,6), pady=6)
    ...
    def disconnect(self):
        try:
            ...
            pass
        # Referansı bırak — sürücü serbest kalmasına yardım eder
        self.client = None
    ...
    def read_temps(self):
        ...
        self.send_and_log(pack(self._addr(), CMD_TMP, bytes([SUB_TMP_MIN])), "TEMP MIN")
        self.send_and_log(pack(self._addr(), CMD_TMP, bytes([SUB_TMP_MAX])), "TEMP MAX")
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        if cmd==CMD_RLY:
            ...
            # Flicker fix: MIN/MAX ve ALL yanıtlarını ayır
            # - ALL: sub=idx(0..19), a1=temp_decic
            # - MIN: sub=SUB_TMP_MIN, a1=idx, a2=temp_decic
            # - MAX: sub=SUB_TMP_MAX, a1=idx, a2=temp_decic
            if sub == SUB_TMP_MIN and len(data) >= 3:
                idx = a1
                val_decic = a2
                if 0 <= idx < 20:
                    self.ntc_min = int(val_decic) * 10
                    self.min_var.set(f"Min: {self.ntc_min/10.0:.1f} °C (N{idx})")
                self._csv_tick()
                return

            if sub == SUB_TMP_MAX and len(data) >= 3:
                idx = a1
                val_decic = a2
                if 0 <= idx < 20:
                    self.ntc_max = int(val_decic) * 10
                    self.max_var.set(f"Max: {self.ntc_max/10.0:.1f} °C (N{idx})")
                self._csv_tick()
                return

            # ALL (tek tek indeks-derece akışı)
            idx = sub
            val_decic = a1
            if 0 <= idx < 20:
    ...
    def on_line_text(self,line:str):
        ...
        if m:
            ...
            self.max_var.set(f"Max: {val:.1f} °C (N{idx})"); self._csv_tick(); return
        m=self._re_min.search(line)
        if m:
            val=float(m.group(1)); idx=int(m.group(2))
            self.ntc_min=int(round(val*10))
            self.min_var.set(f"Min: {val:.1f} °C (N{idx})"); self._csv_tick(); return
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Uygulama kapanışında tüm portları temiz kapat
        self.protocol("WM_DELETE_WINDOW", self._on_close)

        self.after(200,self.scan_ports)

    def _on_close(self):
        # Tüm drawer’ları temiz kapat
