...
- Aydınlık tema
- Same-COM (tek SerialClient, çok adres) modu
- NTC: 0-tabanlı etiket (N0..N19), değiştirilebilir threshold (varsayılan 40.0°C),
       min/max GUI içi hesaplama (MCU MIN/MAX kullanılmaz)
- CSV: Kullanıcı tanımlı saniyelik aralıkta periyodik log (timestamp ile)
       → her turda önce otomatik okuma (NTC ALL), sonra snapshot yazımı
- HEX CSV: Uygulama açıldığı anda otomatik başlar, panele düşen her TX/RX satırı timestamp ile ayrı CSV’ye yazılır
"""
...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        # Spinbox klavye ile değiştiğinde de repaint + MCU set olsun
    ...
    def _build_ui(self):
        ...
        thr_sp = ttk.Spinbox(thrf, from_=self.THR_MIN, to=self.THR_MAX, increment=0.5,
    ...
    def _on_threshold_change(self):
        ...
        # MCU'ya threshold set komutu (AA ADDR 20 02 06 XX CRC 55), XX = tam sayı °C
        try:
            xx = max(0, min(255, int(round(thr))))
    ...
    # NTC tile repaint (flicker önleme — sadece değişiklikte boyama)
    def _set_ntc_tile(self,i:int, val:int):
        ...
        if str(f['bg']) != new_bg:
            f.configure(bg=new_bg)
            l1.configure(bg=new_bg)
            l2.configure(bg=new_bg)
        new_text = f"{deg:.1f} °C"
