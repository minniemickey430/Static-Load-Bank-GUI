...
# ------------------ Protokol ------------------
# Çerçeve: AA | ADDR | CMD | LEN | DATA... | CRC | 55   (CRC8-ATM)
STX = 0xAA
ETX = 0x55
BAUD = 115200

# CMD alanları (yapılan işin türü)
CMD_RLY  = 0x10  # Röle komutları (aç/kapat/sorgu)
CMD_FAN  = 0x20  # Fan komutları + sıcaklık eşik seti (sub=0x06)
CMD_TMP  = 0x30  # Sıcaklık telgrafları (NTC değerleri)
CMD_ADDR = 0x40  # Cihaz adres raporu/değişimi
CMD_EVT  = 0x50  # Sistem olayları (WD reset, hata kodları)
CMD_OLED = 0x60  # OLED ekran uyku/uyan

# Röle
SUB_RLY_ON  = 0x01  # rid açık
SUB_RLY_OFF = 0x02  # rid kapalı
SUB_RLY_QRY = 0x03  # durum sorgu (opsiyonel)
SUB_RLY_ERR = 0xEE  # FW hatası/koruma

# Sıcaklık (GUI kendi min/max’ını hesaplar)
SUB_TMP_ALL = 0x00  # tüm NTC’ler
SUB_TMP_MAX = 0x01
SUB_TMP_MIN = 0x02
SUB_TMP_ONE = 0x03

# Fan (tek açılır menü → tüm fanlara aynı komut)
FAN_LABELS = ["%0", "%25", "%50", "%75", "%100", "AUTO"]
FAN_SUBMAP = {"%0":0, "%25":1, "%50":2, "%75":3, "%100":4, "AUTO":5}
FAN_STATUS_MANUAL_OK = 0xF1
FAN_STATUS_AUTO_OK   = 0xF2
# Not: SUB=0x06 → sıcaklık eşiği seti (DATA: [0x06, XX])

# OLED
SUB_OLED_SLEEP = 0x00
SUB_OLED_WAKE  = 0x01
OLED_DEBOUNCE_MS = 300  # butona çift tıklamaya karşı basit debounce
...
# ------------------ Serial Client ------------------
# Tek porta bağlanan düşük seviye istemci: RX/TX, çerçeve ayrıştırma, satır bazlı ASCII yedek
class SerialClient:
    ...
    def open(self):
        # güvenli yeniden-açma (eski thread/port temizliği)
    ...
    # ---- RX parse ----
    def _emit_text(self):
        # ASCII satırlarını (örn. "NTC[3]=25.1 C") UI'ya aktar
        while True:
    ...
    def _consume_one(self)->bool:
        # AA ... 55 çerçeve ayrıştırma + CRC doğrulama
        buf=self._buf
...
# Tek COM üzerinden çok panel: adres→panel eşlemesi, RX dağıtımı
class SerialHub:
    ...
    def refresh_addr_map(self):
        # anlık panel→addr haritası oluştur
    ...
    def _rx_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        # gelen çerçeveyi adresine göre doğru panele yönlendir
...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        self.relays=[0]*40
        self.ntc_vals=[0]*20           # 1 LSB = 0.1°C *10
        self.ntc_seen=[False]*20
        self.ntc_min=0; self.ntc_max=0

        self.fans=[0,0,0]

        # NTC threshold — varsayılan 40°C (yalnızca INT kabul)
        # Kullanıcı 40’tan farklı girerse WD reset sonrası otomatik geri yüklenir.
        self.THR_MIN = 15
        self.THR_MAX = 80
        self.ntc_threshold_var = tk.IntVar(value=40)   # yalnızca tam sayı
        self.NTC_LIMIT = 400  # 40.0°C (deg*10)
        self.last_valid_thr = 40                        # kullanıcı geçerli en son değer
        self.user_changed_thr = False                  # kullanıcı 40 dışında bir değer verdi mi?

        # Spinbox değiştiğinde repaint + doğrulama + MCU set
        self.ntc_threshold_var.trace_add('write', lambda *args: self._on_threshold_change())

        self._oled_last_ms = 0

        # CSV periyodik loglama (değer CSV'si) — saniye seçimi ile otomatik anlık görüntü
        self.csv_enabled=False
        self.csv_file=None
        self.csv_writer=None
        self.csv_interval_var = tk.IntVar(value=1)  # saniye
        self._csv_job = None
        self._csv_write_job = None
        self.csv_event_mode = False

        # ---------- HEX CSV (otomatik, butonsuz) ----------
        # Tüm TX/RX satırları anlık CSV’ye dökülür; “Open HEX CSV” ile dosya/folder açılır.
    ...
    def _trim_text_lines(self, widget: tk.Text, max_lines:int, keep_lines:int):
        # metin kutularında satır sayısını sınırlı tut (performans)
        try:
    ...
    def open_hex_csv(self):
        # Windows: dosya, macOS/Linux: varsayılan uygulama ile aç
        try:
    ...
    def _build_ui(self):
        ...
        thr_sp = ttk.Spinbox(thrf, from_=self.THR_MIN, to=self.THR_MAX, increment=1,
                             textvariable=self.ntc_threshold_var, width=6,
                             command=self._on_threshold_change)
        thr_sp.pack(side='left')
        ttk.Label(thrf, text="Only integer numbers. Green ≤ threshold < Red").pack(side='left', padx=10)
        ...
        ttk.Button(opt,text="All OFF",style='Danger.TButton').configure(command=self.all_off)
        ttk.Button(opt,text="All ON (static)",style='Accent.TButton').configure(command=self.all_on)
        ...
        # ---- Fans (tek dropdown, tüm fanlar aynı hatta) ----
        ...
        ttk.Label(row,text=f"Fans:",width=10).pack(side='left')
        var=tk.StringVar(value="AUTO")
        cb=ttk.Combobox(row,textvariable=var,state='readonly',width=7,values=FAN_LABELS)
        cb.pack(side='left'); cb.bind('<<ComboboxSelected>>', lambda e:self.on_fan_select(0))
        ...
        # HEX CSV dosyasını aç butonu
        btnrow2 = ttk.Frame(mon); btnrow2.pack(fill='x', padx=8, pady=(4,0))
        ttk.Button(btnrow2, text="Open HEX CSV", command=self.open_hex_csv, style="Tool.TButton").pack(side='left')
    ...
    def _paint(self, idx:int):
        # röle butonlarının rengi (yeşil=açık, kırmızı=kapalı)
        self.relay_btns[idx].configure(bg=('green' if self.relays[idx] else 'red'))

    def _send_threshold_to_mcu(self, thr_int: int):
        """AA ADDR 20 02 06 XX CRC 55 → XX = tam sayı °C"""
        try:
            xx = max(0, min(255, int(thr_int)))
            frame = pack(self._addr(), CMD_FAN, bytes([0x06, xx]))
            self.send_and_log(frame, f"THRESHOLD SET -> {xx}°C (0x{xx:02X})")
        except Exception as e:
            self.log_error(f"[THR] send error: {e}")

    def _resend_threshold_if_needed(self):
        """WD resetten sonra kullanıcı threshold'u 40'tan farklıysa MCU'ya tekrar uygula."""
        if self.user_changed_thr and self.last_valid_thr != 40:
            self._send_threshold_to_mcu(self.last_valid_thr)
            self.set_status(f"OK: Threshold restored ({self.last_valid_thr}°C after reset)")
            self.log(f"[THR] Restored to {self.last_valid_thr}°C after WD reset")

    def _on_threshold_change(self):
        # Kullanıcı sadece int girebilir; küsurat girerse hata ver ve geri al
        try:
            thr = int(self.ntc_threshold_var.get())
        except Exception:
            messagebox.showerror("Threshold", "Only integer numbers.")
            self.ntc_threshold_var.set(self.last_valid_thr)
            return

        # 15–80 aralığına zorla
        if thr < self.THR_MIN or thr > self.THR_MAX:
            thr = min(max(thr, self.THR_MIN), self.THR_MAX)
            self.ntc_threshold_var.set(thr)

        # GUI içi limit ve boyama
        self.NTC_LIMIT = int(round(thr * 10.0))
        self._repaint_all_ntc_tiles()

        # Geçerli threshold'u sakla (WD reset sonrası geri yüklemek için)
        self.last_valid_thr = int(thr)
        self.user_changed_thr = (self.last_valid_thr != 40)

        # MCU'ya threshold set komutu gönder
        self._send_threshold_to_mcu(self.last_valid_thr)
    ...
    def connect(self):
        # Tekil COM bağlantısı (shared mod değilse)
        if self.app.same_port_var.get():
            if not self.app.closing:
                messagebox.showinfo("Shared", "Same COM mode: use top-bar Connect (Shared).")
            return
        ...
        def worker():
            try:
                ...
                self.after(0, lambda: (self.btn_connect.configure(state='normal'),
                                        self.btn_disconnect.configure(state='disabled'),
                                        None if self.app.closing else messagebox.showerror('Connect', f'{e}')))
    ...
    def disconnect(self):
        if self.app.same_port_var.get():
            if not self.app.closing:
                messagebox.showinfo("Shared", "Same COM mode: use top-bar Disconnect (Shared).")
            return
        try:
            if self.client:
                self.client.close()
    ...
    def send_and_log(self, frame:bytes, note:str):
        # TX’i hem HEX paneline yaz hem gönder
        if self.app.same_port_var.get():
    ...
    def set_fans_auto_startup(self):
        # GUI açılınca varsayılan AUTO
        for v in self.fan_vars:
            v.set("AUTO")
        frame = pack(self._addr(), CMD_FAN, bytes([FAN_SUBMAP["AUTO"]]))
        self.send_and_log(frame, "FAN -> AUTO (startup)")

    def on_fan_select(self, idx:int):
        label=self.fan_vars[idx].get()
        code=FAN_SUBMAP.get(label, 5)
        frame=pack(self._addr(), CMD_FAN, bytes([code]))
        self.send_and_log(frame, f"FAN SET -> {label}")
    ...
    def on_click_static_channel(self, ch:int):
        # Statik yük: aynı anda birden fazla açık olabilir (rid=2*k)
        rid = 2 * ch
        idx = rid - 1
    ...
    def on_click_electronic_channel(self, ch:int):
        # Elektronik yük: tek-seçimli; biri açılırsa diğerleri kapanır (rid=2*k-1)
        rid = 2 * ch - 1
        idx = rid - 1
        want_on = (self.relays[idx] == 0)
        if want_on:
    ...
    def all_off(self):
        # FW tarafında 0x00 ile “hepsi kapat” semantiği destekleniyor (SUB_RLY_OFF, rid=0x00)
    ...
    def all_on(self):
        # Sadece statikleri aç; elektroniklerin hepsini kapat
    ...
    # ---- CSV (periyodik, otomatik poll) ----
    def start_csv(self):
        # “Start CSV”: saniye aralığıyla TEMP ALL isteği at, anlık snapshot yaz
        if self.csv_enabled:
        ...
        self._schedule_csv_cycle()

    def _schedule_csv_cycle(self):
        if self._csv_job is not None:
            try: self.after_cancel(self._csv_job)
            except: pass
            self._csv_job = None
        if self._csv_write_job is not None:
            try: self.after_cancel(self._csv_write_job)
            except: pass
            self._csv_write_job = None
        ...
        self._csv_poll()
        self._csv_write_job = self.after(200, self._csv_snapshot_write)

        interval_ms = max(1, int(self.csv_interval_var.get())) * 1000
        self._csv_job = self.after(interval_ms, self._schedule_csv_cycle)

    def _csv_poll(self):
        # Yalnızca TEMP ALL otomatik sorgu (röle sorgusu opsiyonel)
        self.send_and_log(pack(self._addr(), CMD_TMP, bytes([SUB_TMP_ALL])), "AUTO POLL (CSV): TEMP ALL")
        # self.send_and_log(pack(self._addr(), CMD_RLY, bytes([SUB_RLY_QRY, 0x00])), "AUTO POLL (CSV): RLY QRY ALL")
    ...
    def on_frame_bin(self, addr:int, cmd:int, sub:int, a1:int, a2:int, data:bytes):
        ...
        # Sistem olayları (örn. WD reset)
        if cmd==CMD_EVT:
            # System Reset: AA .. 50 02 00 E8 .. 55 → sub=0x00, a1=0xE8 → Independent Watchdog
            if sub == 0x00 and a1 == 0xE8:
                self.log("[EVT] System Reset (Independent Watchdog)")
                for i in range(40):
                    if self.relays[i]!=0:
                        self.relays[i]=0
                        self._paint(i)
                self.set_status("Warning: system reset")
                # Kullanıcı threshold'u 40’tan farklıysa geri uygula
                self._resend_threshold_if_needed()
                return
        ...
        # Bu panelin adresi değilse yok say
        ...
        if cmd==CMD_RLY:
            ...
            # Threshold set yanıtı: sub=0x06, a1=0x00 → OK
            if sub == 0x06:
                if a1 == 0x00:
                    self.set_status("OK: Threshold set")
                    self.log("[FAN] Threshold set ACK (sub=0x06, status=0x00)")
                else:
                    self.set_status(f"Warning: Threshold NACK 0x{a1:02X}")
                    self.log(f"[FAN] Threshold set NACK status=0x{a1:02X}")
            else:
                # Fan mod statüsü
                st=a1
            ...
            # FW per-index cevaplarda sub = 0..19 (N0..N19), a1=deg(0.1°C)
            idx = sub
            val_decic = a1
            if 0 <= idx < 20:
    ...
    def on_line_text(self,line:str):
        ...
        if m:
            ...
            if 0<=i<20:
                ...
                self._set_ntc_tile(i, self.ntc_vals[i])
                self._recompute_minmax()
                return

    # NTC tile repaint (eşik üstü kırmızımsı, altı yeşil)
    def _set_ntc_tile(self,i:int, val:int):
        deg=val/10.0
        f,l1,l2=self.ntc_tiles[i]
        hot=(val>self.NTC_LIMIT)
        new_bg = "#fde2e2" if hot else "#dcfce7"
        if str(f['bg']) != new_bg:
            f.configure(bg=new_bg); l1.configure(bg=new_bg); l2.configure(bg=new_bg)
        new_text = f"{deg:.1f} °C"
        if str(l2['text']) != new_text:
            l2.configure(fg="#111827", text=new_text)
...
class App(tk.Tk):
    def __init__(self):
        ...
        # Same-COM: tek port, çok cihaz (adres bazlı yönlendirme)
    ...
    def shared_connect(self):
        # Same-COM bağlantısı: tek portu aç, tüm panelleri “shared” çalıştır
        if not self.same_port_var.get():
            if not self.closing: messagebox.showinfo("Info", "Enable 'Same COM for all drawers' first.")
            return
        port = self.panels[0].port_var.get().strip()
        if not port or port=="(no ports)":
            if not self.closing: messagebox.showerror('Port','Select a port first')
            return
        try:
            self.hub.open(port)
            for p in self.panels:
            ...
            if not self.closing: messagebox.showerror('Connect (Shared)', f'{e}')
    ...
    # ---- Window close ----
    def _on_close(self):
        # Çıkış: CSV ve seri portları güvenle kapat
        self.closing = True
        try: self.hub.close()
        except: pass
        for p in getattr(self, "panels", []):
            try:
                p.stop_csv()
                p.disconnect()
                p.close_hex_csv()   # HEX CSV güvenli kapatma
            except: pass
