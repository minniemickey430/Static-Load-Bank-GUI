...
class DevicePanel(ttk.Frame):
    ...
    def __init__(self, master, idx:int, app:'App'):
        ...
        # Spinbox klavye ile değiştiğinde de repaint olsun
        self.ntc_threshold_var.trace_add('write', lambda *args: self._on_threshold_change())

        self._oled_last_ms = 0

        # CSV periyodik loglama
        self.csv_enabled=False
        self.csv_file=None
        self.csv_writer=None
        self.csv_interval_var = tk.IntVar(value=1)  # saniye
        self._csv_job = None            # bir sonraki cycle planı
        self._csv_write_job = None      # poll sonrası snapshot yazımı
        self.csv_event_mode = False     # event tetikli mod KAPALI
    ...
    def _schedule_csv_cycle(self):
        ...
        # 1) POLL: okunabilen verileri iste (NTC ALL)
        self._csv_poll()

        # 2) Kısa bir beklemeden sonra SNAPSHOT yaz
        #    (MCU yanıtının GUI'ye düşmesi için 150-250 ms uygundur)
        self._csv_write_job = self.after(200, self._csv_snapshot_write)

        # 3) Bir sonraki cycle'ı planla
    ...
    def _csv_poll(self):
        ...
        # İstersen röle sorgusunu da açabilirsin (FW destekliyorsa):
